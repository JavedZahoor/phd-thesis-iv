...
classifiers = ["AdaBoost","DT","ExtraTree","MLP","SVM","RandomForest"]
...
for dataset in datasets:
	f=open('mcc/mccResults-full'+dataset+'.txt','a');
	f.write("dataset, method, classifier, validationTechnique, mc, timeTaken, cv.max, cv.mean, cv.min, cv.std, preprocessing");
	...
	for method in methods:
		...
		print ("method:",method)
		#first run indices
		#indices= joblib.load('selected_features/dataset'+dataset+'/dataset'+dataset+'_train'+str(size)+'-'+method+'.joblib.pkl')
		X_train=X_train_full
		X_validate=X_validate_full
			...
			for validation in validationTechniques:
				...
				for classifierName in classifiers:
					...
					if(classifierName == "MLP"):
						...
						# Train the model
						grid_clf = RandomizedSearchCV(model,param_grid,cv=validate,iid=False,n_iter=n_iter_search, n_jobs=-1)
						classifier = grid_clf
					...
					for v in validationTechniques:
						...
						f.write("\n "+str(dataset)+", "+str(method)+", "+str(classifierName)+", "+str(v)+", "+str(mc)+", "+str(end_time)+","+str(scores.max()) + "," + str(scores.mean()) + ", " + str(scores.min()) + ", " + str(scores.std() * 2)+", "+str(prepType));
						print("matthew:",matthews_corrcoef(y_validate, y_pred))
						print "Writing file: ", 'dataset'+str(dataset)+'_trained_clfs/'+str(prepType)+"-"+str(v)+"-"+str(classifierName)+'-'+str(method)+'.joblib.pkl'
						joblib.dump(classifier,'dataset'+str(dataset)+'_trained_clfs/'+str(prepType)+"-"+str(v)+"-"+str(classifierName)+'-'+str(method)+'.joblib.pkl')
						sleep(0.2);
