#ref https://github.com/pjmattingly/ant-colony-optimization and modified to suit our needs. contains lots of purely programming/python related learnings that we would not like to redo, hence modifying the following
"""
TODO: JZ20180117
1. Done. Distance to represent cost in our case e.g. inaccuracy value, or percent of accuracy decrease due to this move
2. starting point for all ants to be random instead of a fixed point (nest) which is not applicable in our case
3. Nodes/routes in our case are going to be fully connected graph with cost calculated on the fly for the selected pathway
"""
...
class ant_colony:
	...
	def _get_distance(self, start, end):
		...
		TODO: JZ20180117 think of a way to link this up with cost in our case e.g. error %age? or %age decrease in accuracy due to the latest move so we discourage the paths that have less overall accuracy
		"""
	...
	def _init_ants(self, start):
		...
		TODO: JZ20180117 Change this to start from random locations instead of 0 or a fixed node. will increase probabability of global optima
		"""
